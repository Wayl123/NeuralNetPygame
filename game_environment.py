...
def get_init_state():
  return np.array([PlayerEntity(PLAYER_SIZE, np.array([SCREEN_SIZE[0] / 2, SCREEN_SIZE[1] / 2]), STARTING_ANGLE, PLAYER_SPEED, PLAYER_ROT_SPEED), # player
...
def update_state(action, state):
  ...
  # Player action
  player_move_direction = np.array([0, 0])
  ...
  bullet_list_update = np.array([bullet for bullet in bullet_list_update if time.time() - bullet.start <= bullet.lifespan])
  ...
  return np.array([player, start_time, enemy_list_update, bullet_list_update, enemy_last_spawn, score_update, ray_cast_update]), reward, game_over
...
class MarbleGame:
  ...
  def render(state) -> Image:
    ...
    player, _, enemy_list, bullet_list, _, _ = np.array(state)
