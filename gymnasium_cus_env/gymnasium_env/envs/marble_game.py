...
class MarbleGameEnv(gym.Env):
  ...
  def __init__(self, render_mode=None, size=5):
    ...
    self.observation_space = spaces.Dict(
      {
        "position": spaces.Box(0, self.window_size - 1, shape = (2, ), dtype = np.int32),
        "angle": spaces.Box(-1, 1, shape = (2, ), dtype = np.float32),
  ...
  def _get_obs(self):
    return {"position": self._player.pos, "angle": np.array([math.cos(self._player.angle), math.sin(self._player.angle)], dtype = np.float32), "ray_cast": self._player_ray_cast}
  ...
  def _get_entity_collision(entity : BaseEntity):
  ...
  def _check_collision(colli_1, colli_2):
  ...
  def _check_line_collision(colli, line):
  ...
  def step(self, action):
    ...
    for enemy in enemy_list_update:
      enemy.angle = math.atan2(self._player.pos[1] - enemy.pos[1], self._player.pos[0] - enemy.pos[0])
    ...
    for n in range(RAY_CAST_COUNT):
      ...
      ray_cast_line.append((self._player.pos[0], self._player.pos[1], 
                          self._player.pos[0] + math.cos(angle) * 32, self._player.pos[1] + math.sin(angle) * 32)) # 0 - 32
      ray_cast_line.append((self._player.pos[0] + math.cos(angle) * 32, self._player.pos[1] + math.sin(angle) * 32, 
                          self._player.pos[0] + math.cos(angle) * 64, self._player.pos[1] + math.sin(angle) * 64)) # 32 - 64
      ray_cast_line.append((self._player.pos[0] + math.cos(angle) * 64, self._player.pos[1] + math.sin(angle) * 64, 
                          self._player.pos[0] + math.cos(angle) * 128, self._player.pos[1] + math.sin(angle) * 128)) # 64 - 128
      ray_cast_line.append((self._player.pos[0] + math.cos(angle) * 128, self._player.pos[1] + math.sin(angle) * 128, 
                          self._player.pos[0] + math.cos(angle) * 256, self._player.pos[1] + math.sin(angle) * 256)) # 128 - 256
      ray_cast_line.append((self._player.pos[0] + math.cos(angle) * 256, self._player.pos[1] + math.sin(angle) * 256, 
                          self._player.pos[0] + math.cos(angle) * 512, self._player.pos[1] + math.sin(angle) * 512)) # 256 - 512
    ...
    for enemy in enemy_list_update:
      ...
      for index, line in enumerate(ray_cast_line):
        ray_cast_update[index] = ray_cast_update[index] or self._check_line_collision(enemy_collision, line)
  ...
  def _render_frame(self):
    ...
    player_rotated = pygame.transform.rotate(player_image, math.degrees(self._player.angle))
    ...
    for enemy in self._enemy_list:
      ...
      enemy_rotated = pygame.transform.rotate(enemy_image, math.degrees(enemy.angle))
      enemy_rect.center = enemy.pos

      canvas.blit(enemy_rotated, enemy_rect)

    # Bullet image
    bullet_image = pygame.Surface((4, 8), pygame.SRCALPHA)
    bullet_image.fill(pygame.Color(255, 255, 0))

    for bullet in self._bullet_list:
      bullet_rect = bullet_image.get_rect()

      bullet_rotated = pygame.transform.rotate(bullet_image, math.degrees(bullet.angle))
